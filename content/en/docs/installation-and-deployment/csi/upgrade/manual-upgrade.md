...
## Upgrading CSI from 2.x or 3.x to  4.11.0{#section260220843210}

To upgrade CSI from 2._x_  or 3._x_  to  4.11.0, perform the following operations:
...
5.  Install the backend information backed up in  [2](#li1699321211285)  according to  [Storage Backend Management](/docs/basic-services/storage-backend-management). The command for configuring the storage backend based on the backup backend information is as follows:

    ```
    oceanctl create backend -f configmap.json -i json -n huawei-csi
    ```

## Upgrading CSI from 4.x to  4.11.0{#section173369363219}

To upgrade CSI from 4._x_  to  4.11.0, perform the following operations:
