...
## 配置说明{#section20411174314366}

请根据以下步骤配置使用动态持久卷：

-   [前置准备](#section1173912128225)
-   [配置PVC](#section10939415153713)
-   [使用PVC](#section8172141413917)

## 前置准备{#section1173912128225}

在开始配置动态持久卷前，请参考[配置存储类](/docs/basic-services/storageclass-management/configuring-a-storageclass)，完成StorageClass的配置。
...
<td class="cellrowborder" valign="top" width="43.92439243924392%" headers="mcps1.2.6.1.5 "><a name="ul16793434324"></a><a name="ul16793434324"></a><ul id="ul16793434324"><li><span>RWO/ROX/RWOP：</span>所有类型卷均支持，<span>RWOP</span>需<span>Kubernetes 1.22</span>版本以上支持。Kubernetes 1.29版本以下需要参考<a href="/css-docs/docs/common-o-m-operations/enabling-the-readwriteoncepod-feature-gate">开启ReadWriteOncePod功能门</a>章节开启该特性。</li><li><span>RWX</span>支持情况如下：<a name="ul201701421154515"></a><a name="ul201701421154515"></a><ul id="ul201701421154515"><li>NAS存储：所有卷均支持。</li><li>SAN存储：仅volumeMode设置为Block的卷支持。</li></ul>
</li></ul>
</td>
</tr>
</tbody>
...
>若使用PVC批量创建Pod，Pod长时间处于ContainerCreating状态，且huawei-csi-node服务状态为OOMKilled，说明当前huawei-csi-node服务内存不足。请根据[表1](/docs/appendix/huawei-csi-resource-management#table4106151116363)，将huawei-csi-node内存限制增大。
