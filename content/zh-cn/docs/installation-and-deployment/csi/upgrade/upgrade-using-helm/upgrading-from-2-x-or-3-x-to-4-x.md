...
如果您已按照以上须知评估风险后，确认需要从2.x或3.x版本的CSI升级至4.9.0版本，请按照以下操作步骤备份存储后端配置：
...
请将[备份存储后端配置](#section3200825558)中备份的后端信息，按照[存储后端管理](/docs/basic-services/storage-backend-management)章节的说明配置存储后端，存储后端配置成功后，请务必按照以上须知所述的风险处理方法进行操作，避免Pod在漂移过程中出现问题。
